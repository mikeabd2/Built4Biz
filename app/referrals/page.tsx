...
import { supabase, type Referral, type Profile } from "@/lib/supabaseClient";
import {
  STATUSES,
  STATUS_LABEL,
  STATUS_CLASS,
...
function Referrals() {
  ...
  // Preselect a recipient when arriving from "Refer to …" in the directory.
  useEffect(() => {
  ...
  const given = useMemo(
    () => refs.filter((r) => r.giver_id === user?.id),
    [refs, user]
  );
  const received = useMemo(
    () => refs.filter((r) => r.receiver_id === user?.id),
    [refs, user]
  );
      ...
      {shown.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-line bg-white/50 p-8 text-center mt-6">
          <p className="text-ink/55">
            {tab === "given"
              ? "You haven't passed any referrals yet."
              : "No referrals have come your way yet."}
          </p>
        </div>
      ) : (
        <ul className="mt-5 space-y-3">
          {shown.map((r) => (
            <ReferralRow
              ...
              canEditStatus={tab === "received" || r.giver_id === user?.id}
              onChanged={load}
            />
          ))}
        </ul>
...
function NewReferral({
  ...
  const submit = async () => {
    if (!user) return;
    if (!receiver) {
      setErr("Choose which member this referral is for.");
      return;
    }
    if (!contactName.trim()) {
      setErr("Add the name of the person or business being referred.");
      return;
    }
    setBusy(true);
    setErr(null);
    ...
    <div className="rounded-2xl border border-line bg-white p-5 mt-5 rise">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mt-4">
        <Input
          ...
          placeholder="Who should they reach out to?"
        />
        <Input
          label="Contact phone"
          value={contactPhone}
      ...
      <label className="block mt-4">
        ...
        <textarea
          ...
          rows={3}
          placeholder="Context the member needs: what's needed, timing, how you know them."
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="font-medium text-ink mt-1">
            {referral.contact_name || "Unnamed contact"}
          </p>
          ...
          {referral.description && (
            <p className="text-sm text-ink/60 mt-2">{referral.description}</p>
          )}
        ...
        <div className="shrink-0 text-right">
          {canEditStatus ? (
            <select
              ...
              disabled={saving}
              className={`text-xs rounded-full border px-2.5 py-1.5 outline-none ${
